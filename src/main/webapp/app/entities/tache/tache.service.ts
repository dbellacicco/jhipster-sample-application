...
export class TacheService {
  ...
  protected convertDateFromClient(tache: ITache): ITache {
    const copy: ITache = Object.assign({}, tache, {
      dateCreation: tache.dateCreation && tache.dateCreation.isValid() ? tache.dateCreation.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((tache: ITache) => {
        tache.dateCreation = tache.dateCreation ? moment(tache.dateCreation) : undefined;
      });
    }
    return res;
  }
